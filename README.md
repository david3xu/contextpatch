...
`run_guarded_command` is Stage 2 MCP-only validation support: it interposes no shell, confines argument paths to the repository root, allows only selected validation-oriented programs/subcommands, drains stdout/stderr concurrently, applies a timeout, redacts probable secret values without hiding ordinary paths or docs, and reports command/cwd/exit-code/duration metadata. The default timeout is 120 seconds and the maximum is 600 seconds. It supports repo-relative Python scripts, `pytest`, and a fixed list of named validation shell scripts — `references/check-base-image.sh` with its optional exact `task` argument, plus the argument-free documentation gates `scripts/check-docs.sh`, `scripts/check-doc-commands.sh`, `scripts/docs-audit.sh`, `scripts/check-endpoint-literals.sh`, and `scripts/check-hosted-target-readiness.sh`, and the six root proofs `scripts/front-door-proof.sh`, `scripts/full-platform-proof.sh`, `scripts/prove-auto-workflow.sh`, `scripts/prove-dispatch-preflight.sh`, `scripts/prove-human-ai-team-flow.sh`, and `scripts/prove-local-edition-bundle.sh`. That list is an exact enumeration rather than a `scripts/*.sh` glob, so a file later added to that directory does not become executable by this server. The selected repository root may extend that enumeration for itself with a `.contextpatch/allowed-scripts.json` manifest naming its own argument-free scripts — bounded, validated the same way every other allowlisted argument is, and never shared with any other configured repository. It refuses direct `harbor run` in favor of `harbor_run_start`, along with Docker, `pip`, arbitrary `python -m`, shell scripts outside that combined list, and generic package installation.

Those refusals narrow entry points and arguments. They are not a sandbox, and this README does not claim one. ContextPatch contains no Azure client and no cloud SDK. Child processes inherit the server's environment, so reviewed repository code that an allowlisted program runs (cargo build scripts and tests, npm-family scripts, Python, pytest collection) can read files, start subprocesses, and use the network with the server user's permissions, including any ambient credentials that user holds. npm-family scripts may invoke their own shell even though this server does not. Only `task_image_python_run` carries the documented container isolation with networking disabled. `compose_stack_run` is the deliberate opposite: it starts repository-defined stacks with networking enabled, because a stack proof exists to exercise service-to-service traffic, and it is annotated open-world for that reason. Its containment is narrower than isolation — the compose file is pinned per action, no caller-supplied Docker argument is accepted, and teardown is scoped to this server's own Compose project so it cannot stop an operator's own stack. `artifact_build_check_run` is the packaging gate: it builds a repository Dockerfile and then runs the built image to prove the artifact imports, with the build networked and the import smoke pinned to `--network none` so a dead export cannot be masked by a successful download. Both Docker builds hand work to the Docker daemon, which conventionally runs as root and is not namespaced from the host the way the task image is. The full audit, including the per-action `openWorldHint` classification, is in `docs/execution-threat-model.md`.
