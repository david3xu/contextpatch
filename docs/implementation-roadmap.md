...
| `file_info` | Report file digest, line count, symlink status, and path metadata without ad hoc scripts |
| `list_directory` | List one directory with entry type, symlink, and size metadata without relying on `rg --files` |
...
Guarded commands retain a 120-second default and 600-second general maximum. Exact `harbor run` commands alone may use up to 3600 seconds, and the `dynamo-harbor-task` profile assigns that default to its Harbor steps while keeping its Git and base-image checks lower. The profile reads authoritative per-trial rewards from Harbor's repository-confined `result.json`; rendered tables and progress output are fallback evidence only.
