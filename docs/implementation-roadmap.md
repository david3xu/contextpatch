...
Stage 2A is implemented for the MCP server. Operational resilience includes build provenance, stable `{scratch}` expansion for data/output byproducts, 30/60/120-second direct read/write/Git-and-GitHub reply deadlines with a 16-worker ceiling, a 90-second hard Git subprocess timeout below the outer Git deadline, actionable refusal guidance, cooperative repository/per-file mutation locks, optional complete-file SHA-256 guards for exact replacement, and durable `applied`/`refused`/`unknown`/`interrupted` receipts for exact replacement, exact-hash overwrite, exact untracked deletion, and local commit workflows. Claude Desktop usability includes locked, stale-read-checked, backup-producing maintenance of ordinary `mcpServers` entries, removal of only the exact legacy ContextPatch wildcard `toolPolicy`, and preservation of other policies; runtime tool approval remains owned by Claude host/account/organization policy.
