...
19. If a shell-script validation exception is exposed, it must be fixed to an exact enumeration of named repository scripts, such as `references/check-base-image.sh` and the named documentation gates, never a directory glob and never arbitrary shell authority. A script on that list may take only the arguments the list itself documents, so its own option surface stays out of reach. The enumeration may be extended by a manifest the *selected* repository declares for itself (`.contextpatch/allowed-scripts.json`), so long as each declared entry is itself an exact, argument-free, repo-relative script name subject to the same traversal and path-shape checks as every other allowlisted argument, size- and count-bounded, and refused as a whole rather than silently ignored when malformed. A declaration in one configured repository must never be reachable from another.
...
30. Background task-image, Harbor, validation-profile, Compose-stack, and artifact-build work must
    share a fixed concurrency cap, return stable pollable log ids, and report restart-orphaned
    active work as unknown.
...
`compose_stack_run` is a typed asynchronous stack-proof runner, not general Compose authority. It may start only a named action from a server-owned list, each pinned to exactly one reviewed compose file, and it must derive every Docker argument itself: no caller-supplied compose file, service, project name, mount, or Docker flag is accepted. It must default to a plan, require exact confirmation, refuse an unknown action while naming the available ones, refuse a pinned compose file that is not a regular file in the repository before invoking Docker, and refuse a selected repository because a container receives argv paths rather than a directory descriptor. Every run must use this server's own Compose project name so teardown cannot stop or delete a stack an operator started by hand from the same file, and teardown must be attempted after every outcome with its result reported separately so a leaked stack is visible rather than hidden by a passing proof. It shares the two-job background cap. Unlike the task image, it runs with networking enabled and must be annotated open-world; it is an entry-point restriction, not container isolation.

`artifact_build_check_run` is a typed asynchronous packaging gate, not general Docker build authority. The caller may name a repository Dockerfile, a build context, and arguments for the built image; every Docker flag, the image tag, and the cleanup argv must be derived by the server. Paths must be validated descriptor-relative with no-follow at every component. The image tag must be unique per plan so a concurrent job's cleanup cannot remove another job's image, the smoke arguments must be positioned after the image name so they cannot be reinterpreted as Docker options, the import smoke must be pinned to `--network none`, the smoke must not run when the build failed, and the built image must be removed after any attempt that may have created it with the outcome reported. It must default to a plan, require exact confirmation, refuse a selected repository, share the two-job background cap, and be annotated open-world because the build retains network access and executes repository-authored Dockerfile steps.
