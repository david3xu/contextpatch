...
Use `native_device_run` for bounded simulator/emulator/device smoke actions. It supports listing, creating, booting, running a synced Capacitor iOS app on a target simulator, installing, launching, and reading finite log snapshots for iOS simulators, plus listing/install/launch/logcat actions for Android devices. Use `ios_read_logs` with optional `last` such as `30s` or `2m`; it intentionally does not expose unbounded log streaming. Device-state-changing execution requires `confirm: "run native device"`; raw `xcrun`, `adb`, and package-manager exec access remains unsupported.
