...
Use `native_device_run` for bounded simulator/emulator/device smoke actions. It supports listing, creating, booting, running a synced Capacitor iOS app on a target simulator, installing, launching, and reading logs for iOS simulators, plus listing/install/launch/logcat actions for Android devices. Device-state-changing execution requires `confirm: "run native device"`; raw `xcrun`, `adb`, and package-manager exec access remains unsupported.
