...
`run_guarded_command` is not a shell. It accepts an executable name and argument array, runs from a repo-root-confined working directory, allows only documented validation-oriented programs/subcommands, drains stdout/stderr concurrently, times out, redacts probable secret values without hiding ordinary paths or docs, and returns command/cwd/exit-code/duration metadata. It defaults to 120 seconds and remains capped at 600 seconds. Package-manager script execution is limited to `npm run`/`npm test`, `pnpm run`/`pnpm test`, and `bun run`/`bun test`; install/add/exec-style package-manager commands remain outside this boundary. Read-only Azure inventory and deployment-state reads are allowed — `show`/`list`/`what-if`/`query` forms of `az containerapp` (including `env`, `revision`, `logs`), `az group`, `az account`, `az deployment group`/`sub`, and `az graph query`; every mutating or unknown `az` subcommand, including `create`, `update`, `delete`, `up`, `exec`, and `account set`, stays refused. Azure deploys are owned by typed plan-first tools, not by `run_guarded_command`. Project validation can also run repo-relative Python scripts (`python3 scripts/name.py`), `pytest`, and the scripts on the fixed validation-script list: `references/check-base-image.sh` (optionally with `task`, and available through `base_image_check_run`), plus the argument-free documentation gates `scripts/check-docs.sh`, `scripts/check-doc-commands.sh`, `scripts/docs-audit.sh`, `scripts/check-endpoint-literals.sh`, and `scripts/check-hosted-target-readiness.sh`. Direct `harbor run`, `pip`, Docker, arbitrary `python -m`, shell scripts outside that list, and shell command strings remain refused.
...
Use `base_image_check_run` for `references/check-base-image.sh`, and set `project_path: "task"` for Dynamo/Harbor tasks that require `bash references/check-base-image.sh task`. Shell-script execution is confined to the fixed validation-script list above; nothing here makes arbitrary `bash` or shell snippets available, and a script not on the list is refused with the list named in the refusal.

Use `compose_stack_run` for stack proofs that need the whole topology up. Pick a named action; the compose file and every Docker argument are derived by the server, so there is no place to pass a compose path, service, or Docker flag. It defaults to a plan, needs `confirm: "run compose stack"` to execute, then returns a `log_id` immediately and runs in the background alongside the Harbor/task-image/profile two-job cap — poll it with `read_command_log` rather than restarting. Teardown runs after every outcome and is scoped to the server's own Compose project, so it will not stop a stack you started by hand; `teardown_clean` in the terminal log reports whether it succeeded. This path runs with networking enabled and is not isolated.

Use `artifact_build_check_run` for the packaging lane: it builds a repository Dockerfile and then runs the built image, which is what catches packaging failures and dead exports that reading source cannot. Name the `dockerfile`, optionally a `context`, and `smoke_args` such as `["node","-e","require('.')"]`; the server derives every Docker flag, a unique image tag, and the cleanup. The build has the network, the smoke run does not, and the image is removed afterwards with the outcome reported as `cleanup_clean`. It defaults to a plan and needs `confirm: "run artifact build check"`, then returns a `log_id` to poll.
