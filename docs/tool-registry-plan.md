...
These counts appear throughout and are not the same quantity, so they are reconciled once here.

- **55 registered `NAME` constants** — 54 internal actions plus `project_execute`, the surface wrapper.
- **54 descriptors** in the registry, and 54 definitions in `tools/list` on the full surface. `project_execute` is neither, because it is the wrapper rather than an action.
- **55 wrapper actions** reported by `project_execute` in project mode: the 54 internal actions plus the dispatchable `describe` meta-action.
- **6 per-tool facts** on a descriptor: name, schema, handler, deadline, mutation lock, reach, read-only — counted as six because name and schema are pinned by the same fixture.
- **8 assertion sites** in the pre-refactor codebase, and **5 modules** those sites lived in. The extra three were test-side count assertions rather than source of truth.

### Two classes of evidence, not one

The requirement is that nothing is lost. Inspection cannot establish that across 54 tools, so mechanical checks do it — but they are not all the same kind of check, and the distinction matters when judging how much they carry.

**Behavioural, five facts.** `tools-surface.json` and `tool-matrix.tsv` are generated by running the real code and are compared byte-for-byte. Name and schema come from the advertised surface; deadline, lock, reach, and read-only come from the live classifiers. These are strong: they observe what the server actually does.

**Lexical, one fact.** Handler identity is checked by parsing `registry.rs` as text and requiring each descriptor to invoke `call_<tool_name>`. This is weaker in kind — it enforces a naming convention rather than observing behaviour, and a handler misnamed at its own definition site would satisfy it. It is used because the cheaper alternatives were measured and do not work: function pointers are per-closure and so always distinct, and behavioural discrimination was limited by refusals that did not name their tool. That second constraint has since been removed, so a behavioural check on handler identity is now feasible and is the natural next hardening step.

Three mechanical checks implement the above.

**Golden snapshot, captured before any change.** Serialize all 54 advertised `tools/list` definitions — names, schemas, annotations, descriptions — to a checked-in fixture. Every phase asserts byte-identical output. A refactor that changes one description or drops one annotation fails immediately and names the tool.
