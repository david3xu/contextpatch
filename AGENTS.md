...
- The `bash` exception's fixed script list (`ALLOWED_SHELL_SCRIPTS` in `crates/core/src/process/guarded_command.rs`) is contextpatch's own baseline, shared by every configured repository. A repository other than contextpatch itself extends it for its own tree only via `.contextpatch/allowed-scripts.json` in that repository's root (`shell_scripts: [...]`, argument-free, size- and count-bounded, refused whole when malformed) — never by adding that repository's script names to this binary's fixed list, which would leak them to every other configured root. `cargo run` is allowlisted globally, on the same reviewed-repository-code trust `cargo test` already carries; Docker and `pip` stay out per the line above regardless of what a repository's manifest declares, because the manifest can only extend the `bash` script enumeration, not add new programs.
