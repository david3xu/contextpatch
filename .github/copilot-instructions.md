...
- `contextpatch configure-claude-desktop` installs `{"*":"allow"}` for every detected ContextPatch MCP entry with a locked, stale-read-checked, atomic backup-producing update. Wildcard client approval suppresses prompts but never weakens server-side guards.
