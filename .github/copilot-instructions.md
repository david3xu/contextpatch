...
| `cli` | Human-facing command names, help text, exit codes, terminal output, safe Claude Desktop policy configuration | Edit semantics or duplicated safety logic |
...
- `replace_exact.expected_sha256` is an optional complete-file concurrency guard; when supplied, a stale or malformed digest must refuse before mutation.
...
- `apply_patch` and `insert_at_anchor` remain Stage 2 boundary tools, not advertised MCP capabilities. The CLI `apply-patch` command is still a not-implemented stub; do not infer support from names in the source or tool spec.
- `move_tracked` moves one clean tracked regular file through guarded `git mv` with an absent destination, clean-index, dry-run, confirmation, symlink, and hash-preservation checks. `delete_guarded` deletes one clean tracked regular file only with its current lowercase SHA-256, dry-run, and exact confirmation, leaving an unstaged tracked deletion.
- `run_guarded_command` is validation support, not a shell: program plus args only, repo-root-confined cwd, timeout, redaction/truncation, null stdin, and allowlisted command families. It supports validation-oriented `git`, Rust, JS package-manager scripts, repo-relative Python scripts, `pytest`, `harbor run`, a fixed enumeration of named validation shell scripts (`ALLOWED_SHELL_SCRIPTS` in `crates/core/src/process/guarded_command.rs`), and `rg`; exact `harbor run` may use up to 3600 seconds while every other command remains capped at 600. It must not expose shell strings, shell scripts outside that enumeration, Docker, `pip`, arbitrary `python -m`, or package installation as generic execution. Extend the shell exception by adding a named script to that list — never by globbing a directory, which safety-contract clause 19 refuses.
- Guarded command data/output arguments may use `{scratch}` for stable repository-specific byproducts outside the worktree; traversal outside that scratch root remains refused, and Python script selection cannot use the token.
- Direct MCP reads, writes, and Git/GitHub operations have 30/60/120-second reply deadlines with at most 16 active workers. Expiry means outcome unknown because the worker is not cancelled; use `read_write_receipts` and current-state tools before retrying.
- ContextPatch mutations are cooperatively serialized per repository, and compare-and-write file operations hold per-target locks; external programs that ignore advisory locks can still race.
- Receipt coverage is explicit: exact replacement, exact-hash overwrite, exact untracked-file deletion, and exact/scoped/prefix local commits are currently journaled with `applied`, `refused`, `unknown`, or `interrupted` outcomes.
- Docker stays out of `run_guarded_command` entirely. Every Docker path is a typed tool whose argv is derived in `core` and executed through `run_bounded_command`: `task_image_python_run` and `image_cleanliness_check_run` are isolated with networking disabled, `docker_image_inspect` is read-only, `compose_stack_run` starts a named stack proof with networking enabled, and `artifact_build_check_run` builds a repository Dockerfile then runs the built image with the smoke half pinned to `--network none`; both are annotated open-world. Add a Docker capability by adding a typed action with a pinned plan, never by allowlisting `docker`.
...
- `contextpatch configure-claude-desktop` keeps detected ContextPatch MCP entries in Claude Desktop's ordinary `mcpServers` map, removes only the exact legacy ContextPatch wildcard `toolPolicy: {"*":"allow"}`, preserves every other policy and unrelated data, and uses a backup, cooperative lock, stale-read check, and atomic replacement. It never claims local prompt suppression: Claude host/account/organization policy owns runtime approval.
