...
The Stage 1 edit tools (`read_range`, `diff_preview`, `replace_exact`, `write_new_file`, `status_guard`) exist across core, CLI, and MCP server. Stage 2A validation/Git workflow tools are MCP-facing: capability/preflight, guarded commands/logs, validation profiles, exact local commit, remote check, branch preparation, merge readiness, and exact push.
...
- `write_new_file` is create-only; `diff_preview` previews exact replacement without mutating.
...
- `run_guarded_command` is validation support, not a shell: program plus args only, repo-root-confined cwd, timeout, redaction/truncation, null stdin, and allowlisted command families.
- Local/remote Git mutation is intentionally narrow: `git_commit_exact` is exact-path local-only; remote publishing is split across `git_remote_check`, `git_branch_prepare`, `git_merge_readiness`, and `git_push_exact`.
