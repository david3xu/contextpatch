...
- `write_new_file` and `write_new_file_base64` are repo-root-confined create-only tools; `artifact_write_text` and `artifact_write_base64` are create-only sidecar artifact tools outside the repo tree; `diff_preview` previews exact replacement without mutating.
...
- `run_guarded_command` is validation support, not a shell: program plus args only, repo-root-confined cwd, timeout, redaction/truncation, null stdin, and allowlisted command families. It supports validation-oriented `git`, Rust, JS package-manager scripts, repo-relative Python scripts, `pytest`, `harbor run`, and `rg`; it must not expose shell strings, Docker, `pip`, arbitrary `python -m`, or package installation as generic execution.
